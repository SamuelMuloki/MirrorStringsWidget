...
define([
    ...
    return declare("MyFirstWidget.widget.MyFirstWidget", [ _WidgetBase, _TemplatedMixin ], {
        ...
        reverse_func:function(m){
            logger.debug(this.id + ".reverse_func");
            var str = [];
            if(this.textString.value !== ''){
                 str = this.textString.value;
            }
           
            for( var x=str.length; x>=0; x--){ 
                _arrayString += arr[x];
                //document.write(_arrayString);
        }
        alert(this._arrayString);
            
        },

        show:function(){        
                if(this._arrayString !== ""){
                    alert(this._arrayString);
                }else{
                    alert(this.colorSelectNode.value);
                }
            },
